    ...
    for _ in range(try_times):
        try:
            ...
            response = requests.post(
                'https://m.nuaa.edu.cn/ncov/wap/default/save', data=data, cookies=cookies)
...
def send_result(recever, result, messgae):
    ...
    smtp_password = '**********'
    smtp_host = '**********'
    print("[+] start send by {} to {} and message is {}".format(mail_sender, recever, messgae))
    if result == True:
...
def check_one_person(student):
    ...
    # if mail != '':
        # print("start sending mail to {}".format(mail))
        # send_result(mail, result, message)
